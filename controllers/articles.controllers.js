...
exports.findAll = async (req, res) => {
            ...
            try {
                ...
                if (userId) {
                    if (country === "ng") {
                        ...
                        // If an error occurs get articles from db instead
                        Article.paginate({country, hiddenBy: {$nin: [userId]}}, { page: page, limit: 10, sort: { publishedAt: -1 } })
                        .then(article => {
                            res.send(article);
                        }).catch(err => {
                            res.status(500).send({
    ...
    // if getting more pages of data, only get from the database not from the API
    else if (page > 1) {
        if (userId) {
            if (country === "ng") {
                // If an error occurs get articles from db instead
